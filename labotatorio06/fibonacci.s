...
@ -- Fibonacci
@entrada: O valor da sequencia que deve ser achado esta em r0
@saida: o resultado final buscado se encontra em r1

fibonacci:
        sub r0, r0, #2 @subtrai 2 do contador, ja que os 2 primeiros termos da sequencia sao 1
        mov r1, #1
        mov r2, #1
        cmp r0, #0
        ble fim
...
loop_fib:
        stmfd sp!, {lr} @empilha lr para comecar a funcao
        sub r0, r0, #1  @subtrai um do contador
        mov r3, r1      @move r1 para r3 (auxiliar)
        add r1, r1, r2  @soma r1 com r2 e salva em r1 (termo da sequencia atual)
        mov r2, r3      @move r3 para r2 (desocupa o auxiliar e coloca o termo de volta na sequencia)
        comp r0, #0     @compara se ja chegou no numero que queria da sequencia
        bgt loop_fib    @se ainda nao chegou, volta em mais uma chamada recursiva
        ldmfd sp!, {lr} @recupera lr
        mov pc, lr      @retorna

fim:
        mov r0, r1 @coloca o resultado do numero procurado em r0
